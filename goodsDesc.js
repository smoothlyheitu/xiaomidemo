define(['jquery','jquery-cookie'],function($){
	...
	function banner(){
		...
		//点击按钮完成切换  事件委托
		$('body').on('click','#J_buyBox .ui-controls .ui-pager .ui-pager-item a',function(){
			//获取当前点击的a标签父节点在兄弟节点中的下标
			iNow = $(this).parent().index();
			tab();
			return false;
		})
		//添加移入移出效果
		$('body').on('mouseenter','#J_buyBox #J_img',function(){
			clearInterval(timer)
		})
		$('body').on('mouseleave','#J_buyBox #J_img',function(){
			timer = setInterval(function(){
				iNow++;
				tab()
			},1000)
		...
		//添加上一张和下一张按钮点击后的切换
		$('body').on('click','#J_buyBox .ui-prev,.ui-next',function(){
		...
		//给加入购物车，添加点击操作
		$('body').on('click','#J_buyBox .J_login',function(){
			//获取当前点击加入购物车按钮，商品id
			//cookie本地缓存{最大4KB、只能字符串}：存储商品的id，商品的数量

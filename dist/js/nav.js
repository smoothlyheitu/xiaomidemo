	...
	function download(){
		...
		$.ajax({
			...
			success : function(result){
				var bannerArray = result.banner;
				for(var i = 0;i<bannerArray.length;i++){
					$(`<a href="${bannerArray[i].url}">
								<img class="swiper-lazy swiper-lazy-loaded" src="./images/banner/${bannerArray[i].img}" >
	...
	//实现轮播图效果
	 function banner(){
		 ...
		 function tab(){
			 ...
			 if(!aBtn){
				 aBtn = $('#J_homeSwiper .swiper-pagination').find('a')
			 }
			 if(iNow == 5){
				 iNow = 0;
			 }
			 aImg.hide().css('opacity',0.2).eq(iNow).show().animate({
				 opacity : 1
			 },500)
			 aBtn.removeClass('swiper-pagination-bullet-active').eq(iNow).addClass('swiper-pagination-bullet-active')
		 }
		 //添加鼠标移入移出操作
			 ...
			 success : function(result){
				 ...
				 for(var i = 0;i < sideArr.length;i++){
					 ...
					 <li class = 'category-item'>
					     ...
					     <div class="children clearfix" style = ''>
					     </div>
					 </li>`)
	 ...
	 function topNavTop(){
		 $('.header-nav .nav-list').on('mouseenter','.nav-item',function(){
		  ...
		  // $('#J_navMenu').on('mouseleave',function(){
